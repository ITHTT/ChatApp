...
@MessageTag(value = "app:medaivideo", flag = MessageTag.ISCOUNTED | MessageTag.ISPERSISTED, messageHandler = VideoMessageHandler.class)
public class MideaVideoMessage extends MessageContent {
    ...
    public MideaVideoMessage(byte[] data) {
    ...
    public MideaVideoMessage() {

    }

    private MideaVideoMessage(Uri thumbUri, Uri localUri) {
        mThumUri = thumbUri;
        mLocalUri = localUri;
    }

    private MideaVideoMessage(Uri thumbUri, Uri localUri, boolean original) {
        mThumUri = thumbUri;
        mLocalUri = localUri;
        mIsFull = original;
    }
     ...
     */
    public static MideaVideoMessage obtain(Uri thumUri, Uri localUri) {
        return new MideaVideoMessage(thumUri, localUri);
    }
     ...
     * @return ImageMessage对象实例。
     */
    public static MideaVideoMessage obtain(Uri thumUri, Uri localUri, boolean isFull) {
        return new MideaVideoMessage(thumUri, localUri, isFull);
    }
     ...
     * @return ImageMessage对象实例。
     */
    public static MideaVideoMessage obtain() {
        return new MideaVideoMessage();
    }
    ...
    public byte[] encode() {
        ...
        try {
            if (!TextUtils.isEmpty(mBase64)) {
                ...
                Log.d("MideaVideoMessage", "base64 is null");
            }
     ...
     */
    public MideaVideoMessage(Parcel in) {
     ...
     */
    public static final Creator<MideaVideoMessage> CREATOR = new Creator<MideaVideoMessage>() {

        @Override
        public MideaVideoMessage createFromParcel(Parcel source) {
            return new MideaVideoMessage(source);
        }

        @Override
        public MideaVideoMessage[] newArray(int size) {
            return new MideaVideoMessage[size];
        }
    };
}